...
function notify_err() {
    log(index.kasm_default_network)
...
function enable_lossless () {
    if [ "${ROLE}" == "app" ] || [ "${ROLE}" == "all" ]; then
        ...
        sed -i "/Cache-Control/a \                add_header              'Cross-Origin-Resource-Policy' 'same-site';" ${KASM_INSTALL_BASE}/conf/nginx/services.d/kasmguac.conf
    fi
}
